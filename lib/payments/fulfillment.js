...
import { normalizeOrderItems, isTracked, deriveStockStatus } from "../inventory.js";

// Decrement tracked variants for a freshly created (non-duplicate) order.
// Untracked variants (inventory_count NULL) are left alone. Best-effort per
// line — an inventory hiccup must never unwind a paid order — but every
// outcome is recorded in audit_logs so drift is visible, not silent.
async function decrementInventory({ items, orderNumber }) {
  for (const line of normalizeOrderItems(items)) {
    try {
      const { data: v } = await supabaseServer
        .from("product_variants")
        .select("id, sku, inventory_count, low_stock_threshold, stock_status")
        .eq("sku", line.sku)
        .maybeSingle();
      if (!v || !isTracked(v)) continue;

      const next = Math.max(0, Number(v.inventory_count) - line.quantity);
      const status = deriveStockStatus(next, v.low_stock_threshold);
      await supabaseServer
        .from("product_variants")
        .update({ inventory_count: next, stock_status: status })
        .eq("id", v.id);
      await supabaseServer.from("audit_logs").insert({
        ...
        metadata: {
          ...
          from: v.inventory_count,
          to: next,
          stock_status: status,
          oversold: Number(v.inventory_count) < line.quantity,
        },
      });
    } catch (e) {
      console.warn("Inventory: decrement skipped for", line.sku, e?.message || e);
    }
  }
}
...
// Exported for scripts/test-attestation-record.mjs (opt cycle 2).
export async function logCheckoutAttestation({ userId, orderNumber, ip }) {
  if (!userId) return;
  try {
    const { data: p } = await supabaseServer
      .from("profiles")
      .select("attestation_version, attestation_statements, attestation_legal_name, attestation_ip, attestation_user_agent")
      .eq("id", userId)
      .maybeSingle();
    ...
    await supabaseServer.from("attestation_audit").insert({
      ...
      // Opt cycle 2 (4.1): the user agent captured at attestation time. The
      // webhook's own UA is the processor's, so the profile snapshot is the
      // right source — same as attestation_ip above.
      user_agent: p.attestation_user_agent || null,
...
// Bind the pre-payment compliance record (written by api/checkout-compliance.js
// before the user reached payment) to the order it authorized, in BOTH
// directions: order_attestations.order_id -> order number, and
// orders.compliance_id -> the record. Best-effort and fully optional: orders
// placed through the legacy single-step checkout carry no compliance_id, and a
// database that hasn't run scripts/proposed-order-attestations.sql simply skips
// this — neither case may ever fail a paid order.
async function linkComplianceRecord({ complianceId, orderNumber }) {
  const id = Number(complianceId);
  if (!Number.isInteger(id) || id <= 0) return;
  try {
    await supabaseServer
      .from("order_attestations")
      .update({ order_id: orderNumber })
      .eq("id", id);
    await supabaseServer
      .from("orders")
      .update({ compliance_id: id })
      .eq("order_number", orderNumber);
  } catch (e) {
    console.warn("Compliance link skipped:", e?.message || e);
  }
}
...
export async function fulfillOrder({
  ...
  await linkComplianceRecord({ complianceId: consent.compliance_id, orderNumber });
  await decrementInventory({ items, orderNumber });
  ...
  try {
    await sendOrderConfirmationEmail({
      ...
      items,
      shippingAddress,
      shippingMethod: consent?.shipping_method || null,
      customerName,
    });
  } catch (err) {
