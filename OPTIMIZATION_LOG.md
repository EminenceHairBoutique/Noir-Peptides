...
| 4.7 | Performance | 9 | 9 → **8 after merge** | the nine rested on a **deterministic** lane (the loader race fixed, H-017; hydration time-sliced; seven URLs gated; PR-head Evidence green twice). After the merge the `main` Evidence run read `/` TBT 557 then 320 ms on two attempts while every other URL stayed under 41 ms — real, undiagnosed from the summary — so the card is 8 until a `main` run is green (cycle 13, lead #1). 10 = live Lighthouse on the real host |
...
| 4.7 Performance | **8** (after merge) | a green Evidence run on `main` — the merge commit's run is red on `/` TBT twice (557 / 320 ms; the same tree read 0 on both PR-head runs 34810277084 / 34815199851 and 3 ms locally); cause not diagnosable from `ci/latest.json` | live Lighthouse (D4 + first probe; the live PDP's ~150 s TBT) | CI: PR-head Evidence green (7 URLs: LCP 1655–1660, TBT ≤ 80) · `main`: run 34828821181 red on `/` only, attempts 1 + 2 · live: `/shop` 28 · `/test-results` 165 · PDP 149 681 ms |
