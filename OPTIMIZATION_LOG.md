...
| 4.7 | Performance | 9 | 9 (dipped to 8 for a day) | the nine rested on a **deterministic** lane (the loader race fixed, H-017; hydration time-sliced; seven URLs gated; PR-head Evidence green twice). After the merge the `main` Evidence run read `/` TBT 557 then 320 ms on two attempts while every other URL stayed under 41 ms — real, undiagnosed from the summary — so the card was 8 until a `main` run came back green — run 35017309553 on `d96cc6e` did (`/` TBT 2 ms), and five more data points showed the spike is bimodal, not a regression, so the card is **9** again with the spike carried as a cycle-13 instrument task. 10 = live Lighthouse on the real host |
...
| 4.7 Performance | **9** (restored 2026-09-15) | — (Evidence green on `main` `d96cc6e`, run 35017309553: `/` TBT 2 ms, every URL in budget) | live Lighthouse (D4 + first probe; the live PDP's ~150 s TBT) | CI: PR-head Evidence green (7 URLs: LCP 1655–1660, TBT ≤ 80) · `main`: run 34828821181 red on `/` only, attempts 1 + 2 · live: `/shop` 28 · `/test-results` 165 · PDP 149 681 ms |
...
- **Evidence on `main`** ([34828821181](https://github.com/EminenceHairBoutique/Noir-Peptides/actions/runs/34828821181), the merge commit): `ci/latest.json` for `aef0995` **red on Lighthouse `/` only — TBT 557 ms (median of 3), LCP 1507**; the other six URLs 0–125 ms TBT, screens 372 / 0, axe 0, crawls green. The same tree read `/` TBT 0 on both PR-head runs (34810277084, 34815199851). Failed job re-run once (attempt 2, 09:57–10:15Z): **red again — `/` TBT 320 ms (LCP 1661), every other URL 0–41 ms** (`ci/aef0995….json`). A second failure is real. Both PR-head runs of the same tree and the local lane read `/` 0–3 ms; the cause is not readable from the summary (the per-route reports live only in the 167 MB artifact). **Under H-014 this takes 4.7 back to 8** until a `main` Evidence run is green — cycle-13 lead #1, with the breakdown publishing (lead #2) as its instrument.

- **The probe's own verdict gate could never pass (found 2026-09-14 22:05Z from the owner's screenshot of run #4; fixed here).** Cycle 12 split `live-probe.yml` into a read-only probe job and a write-capable publish job (`79bbf4c`) but left the final guard — `if: steps.verdict.outcome != 'success'` — in the publish job, while `id: verdict` stayed in the probe job. A `steps.<id>` reference does not cross a job boundary: in `publish` it reads as the empty string, `'' != 'success'` is always true, so **the step always ran and the run could never be green**; the sibling `continue-on-error: true` steps meanwhile made the probe job always green. Runs #1–#4 all reported "failure" and the conclusion carried no information — the site happened to be red as well, which is why four runs passed unexamined ([#4](https://github.com/EminenceHairBoutique/Noir-Peptides/actions/runs/34901239860): probe job green in 11 m 31 s, publish job red in 18 s). The gate now reads the RECORD the publish job downloaded (`node scripts/live-probe.mjs --gate evidence/live-probe.json`): green → 0, red / incomplete / missing / unreadable → 1 with the failing checks named and the host-config reds counted apart. `scripts/test-live-probe-gate.mjs` executes all seven cases (H-018: prove both poles), and `scripts/test-workflow-shape.mjs` catches the whole class — a `steps.<id>` reference outside its declaring job, or a `needs:` naming no job — in every workflow, proven against the exact shape that shipped. **No scorecard moves:** the record is red on its own merits (host config, rails, the PDP's TBT), so 4.12 stays 8; what changes is that a green run is now reachable at all.

- **The gate written to enforce H-018 could not fail either (same night, adversarial panel).** A verification workflow over that commit — 8 reader lenses, 26 candidate findings, each judged by three refuters from different angles, 7 confirmed — found that `test-workflow-shape.mjs` recognised job keys only at exactly two-space indentation: a four-space workflow (valid YAML) yielded zero job blocks, zero blocks yielded zero findings, and the sweep printed a tick over a file carrying both defect shapes. Also confirmed: an `id:` was registered from any line, so a heredoc inside `run: |`, a `with:` input or a job-level `env:` named `id` masked a real cross-job reference; a trailing `# comment` on an `id:` or `needs:` line made the checker accuse correct code; and a block-list `needs:` — the standard shape for two upstreams — was never checked at all. All fixed in `36c4574` with a fixture per finding (the real pre-fix `live-probe.yml` is now tracked under `scripts/__fixtures__/`), plus the meta-assertion the first draft lacked: a file that declares `jobs:` and parses to none is a FINDING, and the sweep pins `live-probe.yml` = probe, publish and `evidence.yml` = evidence, publish, so a reformat that blinds the parser turns the gate red instead of quiet. The probe's own `--gate` was hardened the same way: a record the finalize step never folded can read `verdict: "green"` from the HTTP phase alone with axe and Lighthouse never run, so all three gate keys are now required. 22 + 15 executed assertions; `test:unit` 1534.

- **Both fixes proven on the live site (2026-09-15, run [#8](https://github.com/EminenceHairBoutique/Noir-Peptides/actions/runs/35016076194), dispatched on `main` right after #47 merged as `6948963`).** The publish job's last step now reads:
  `live probe: red (27/37 checks) — failing: rails available, lighthouse — host config: 9 check(s), set PROD_URL / CANONICAL_HOST`
  followed by the `::error::` annotation — i.e. the run's verdict comes from `scripts/live-probe.mjs --gate` reading the published record, naming what failed, instead of the cross-job `steps.verdict.outcome` expression that had made every run red regardless of the site. A green record would now exit 0 (proven at that pole by `test-live-probe-gate.mjs`, which is what a live green run cannot demonstrate until the site is green). **Second proof in the same line:** the count moved 26/37 → 27/37 and `scanner /shop` has left the failing list — the `scanText` parity fix (`92a4c41`) works against production, not just against `serve-dist`. The remaining reds are the site's own and unchanged: 9 host-config (D4), `rails available: []` (D8), and the live PDP's Lighthouse TBT.

- **The `/` blocking-time red is a bimodal spike, not a regression (four data points, 2026-09-14/15).** Evidence on `main` read `/` TBT 557 ms (attempt 1) then 320 ms (its one re-run); PR #48, a two-file docs change, read 483 ms with per-run values of **1373 / 483 / 0 ms**; the re-run of that same job on the same commit came back **0 ms, every URL green** ([run 35018339301](https://github.com/EminenceHairBoutique/Noir-Peptides/actions/runs/35018339301), `ci/latest.json` verdict green). The immediately preceding head of the same tree (#47's `5ea17c1`) was green too. So the landing page occasionally pays a ~1.4 s main-thread block in the CI runner and usually pays none — a spike to diagnose, not a change to revert, and a three-run median is the wrong instrument for it (H-017: read every run). **4.7 stays 8** on the standing rule (a card is 9 only while every CI check is green on `main`, and `main`'s own Evidence run is red) until a `main` run is green — the next push to `main` produces one. Cycle-13 lead #1 keeps its instrument: publish the per-route long-task and bootup breakdown into `summary.json`, and raise the run count on `/` so the spike is visible rather than averaged.

- **4.7 back to 9 — an Evidence run on `main` is green (2026-09-15).** Run [35017309553](https://github.com/EminenceHairBoutique/Noir-Peptides/actions/runs/35017309553) on `d96cc6e`: verdict green, `/` **TBT 2 ms** (LCP 1655), `/shop` 77, PDP 39, `/test-results` 18, category 3, permalink 3, `/partners` 0; screens 372 / 0, axe 0, crawls green. That satisfies the condition this log set when the card was marked down ("9 again when a `main` Evidence run is green"), so **4.7 = 9** on CI evidence, with the `/` spike still open as a cycle-13 instrument task (it is a flake to make visible, not a budget the site fails).
- **A fail-soft catch hid a broken distributed rate limiter — the engine's gates never saw it (owner fix `73bc6a0`, 2026-09-15).** `api/_utils/rateLimit.js` selected `id, request_count, window_start` from `rate_limits`, a column the table does not have; PostgREST answered with an error, and the handler's `catch` degraded to the in-memory backstop by design. On serverless that means every instance kept its own counter, so the limit was far weaker than the number in the code — and nothing anywhere went red. None of the engine's gates cover it: `db-shape-diff.mjs` compares only products / variants / tiers, `test-rate-limit-coverage` proves which endpoints CALL the limiter rather than that the call works, and the harness stubs never execute the real select. **Cycle-13 item:** gate the columns each server module selects against the migration-defined schema (parse the `select("…")` strings, check them against `docs/SCHEMA.md` / the migrations, or probe them on the db-gates Supabase stack), and treat "degrade silently on error" paths as gate-worthy: a fail-soft branch that is never observed is indistinguishable from a broken feature (H-016's shape, one level down).
