...
// Google Fonts host — those cause a request. Since opt cycle 9 (C3) the CSP
// no longer lists the hosts as allowed origins either (asserted below).
...
ok(!/fonts\.(googleapis|gstatic)\.com/.test(buildCsp({ analyticsEnabled: true, allowInlineScript: true, forHeader: true })), "CSP style-src / font-src name no Google Fonts host (opt cycle 9, C3)");
ok(!/fonts\.(googleapis|gstatic)\.com/.test(fs.readFileSync(path.join(process.cwd(), "public/sw.js"), "utf8")), "service worker has no font-CDN cache branch");
