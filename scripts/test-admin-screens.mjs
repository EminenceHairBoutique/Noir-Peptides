...
console.log("\nOwner Sprint panel:");
{
  ...
  let m = byId(d.rows);
  ok(d.rows.length === 12 && d.rows.map((r) => r.id).join() === "D1,D2,D3,D4,D5,D6,D7,D8,D9,D10,D11,D12", "twelve rows D1–D12 in order");
  ...
  d = await deriveOwnerSprint({ VITE_SITE_URL: "https://www.noirpeptides.com", BTCPAY_URL: "https://btc.example", BTCPAY_API_KEY: "k", BTCPAY_STORE_ID: "s", BTCPAY_WEBHOOK_SECRET: "w", VITE_GA_MEASUREMENT_ID: "G-1" });
  m = byId(d.rows);
  ...
  ok(r.statusCode === 200 && r.payload.rows.length === 12, "GET → 200 with the twelve rows");
