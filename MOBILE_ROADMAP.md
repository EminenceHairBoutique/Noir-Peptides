...
| 9 | ~~**Swipe gallery**~~ — **✅ DONE (eighth pass).** `src/components/product3d/MediaGallery.jsx`: CSS scroll-snap (no gesture lib), three slides — interactive 3D vial · flat front panel · full-wrap label — **all renders of the approved label config** (the catalog has zero product photos; nothing is invented). Dot navigation (absolute inside the square panel → zero CLS; late-label CLS re-measured 0.0002), `aria-roledescription` carousel/slide semantics, `prefers-reduced-motion` honored on dot scrolls. Swiping ON the vial rotates it by design; dots/edges change slides. No label → no gallery chrome. Found+fixed while testing: the lg caption overlay intercepted pointer events over the dots (`pointer-events-none`) | Natural touch interaction; the media column already supports multiple views | S | **Done** |
