...
| 4 | **Skeleton loading states** — **mostly already existed; improved this pass.** The catalog grid skeleton was already correct (**measured CLS = 0**). The PDP skeleton was a short centered block and now mirrors the real page structure; `se-skeleton` shimmer now respects `prefers-reduced-motion`. **A separate PDP shift (CLS ≈ 1.0 at ~12.5s, footer) remains OPEN — root cause not identified**, see MOBILE_AUDIT.md | Grid was fine; PDP loading state was thin | S | Partial |
