...
/**
 * Build ONE print-ready PDF containing a page per label config — the file
 * that goes to the printer for a production run. Each page is generated by
 * labelPdfBlob (identical output to the per-label export; nothing new can
 * drift) and merged in catalog order. Sequential on purpose: each label
 * rasterizes at print DPI and parallel canvases would exhaust mobile memory.
 *
 * @param {Array<import('./types.js').ProductLabelConfig>} configs
 * @param {{siteUrl?: string, dpi?: number, onProgress?: (done:number, total:number, label:string)=>void}} opts
 * @returns {Promise<Blob>}
 */
export async function labelBatchPdfBlob(configs, opts = {}) {
  const { PDFDocument } = await import("pdf-lib");
  const list = Array.isArray(configs) ? configs : [];
  if (!list.length) throw new Error("labelBatchPdfBlob: no labels to export");

  const merged = await PDFDocument.create();
  let done = 0;
  for (const config of list) {
    const blob = await labelPdfBlob(config, {
      templateId: config.template_id,
      presetId: config.default_preset || "full_wrap",
    ...
    opts.onProgress?.(done, list.length, `${config.display_name || ""} ${config.quantity_label || ""}`.trim());
  }
